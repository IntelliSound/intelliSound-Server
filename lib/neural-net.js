...
const predictor = (dataToProcess, network = myNetwork) => {
  ...
  for(let kalvin = 0; kalvin < 1; kalvin ++){
    for (let i = 0; i < arrayToProcess.length - 128; i++){

      const currentValues = [];
      for(let j = 0; j < 16; j++){

        currentValues.push(dataToProcess.neuralTransformedArray[i + (j * 8)]);

      }
      network.activate([...currentValues]);
      network.propagate(LEARNING_RATE, [i + 128]);
    }
    // let arr = new Float64Array(1);
    // arr[0] = .01;
    // console.log(arr);
    // network.optimized.memory = arr;
    console.log(network.optimized.memory);
  ...
  for (let i = 0; i < NEURAL_PARSE_LENGTH; i++) {
    ...
    if(i === 0){
      ...
      console.log(dataToProcess.neuralTransformedArray[128]);
    }
  }
